...
def instantiate_slice_subnet(data_json):  
    ...
    #return response.text, response.status_code
    logging.debug('Sending request to deploy slice-subnet: ' + str(data_dumps))
    id_sample = str(uuid.uuid4())
    return {"id": id_sample}, 200
...
def get_slice_subnet_instance_request(request_ID):
    ...
    #response = requests.get(url, headers=JSON_CONTENT_HEADER)
    #return response.text, response.status_code
    logging.debug('Getting slice-subnet info based on request: ' + str(request_ID))
    sample_json = {}
