...
mutex_transport2blockchaindb_access = Lock()
...
def update_terminate_from_blockchain(event_json):
    ...
    for e2e_cs_item in e2e_cs_list:
        ...
        if found_cs == True:
            break
    if found_cs == False:
        settings.logger.info("ORCH: ERROR - Domain CS not found, error in code that created the E2E CS['domain-cs'].")
    mutex_e2e_csdb_access.release()
...
def instantiate_e2e_connectivity_service(e2e_cs_request):
    ...
    mutex_e2e_csdb_access.release()
    if found_error == True:
        settings.logger.info("ORCH: Found error in deployed CS, undoing the actions done to release the resources.")
        ...
        #TODO: release blockchain resources that were selected
...
def terminate_e2e_connectivity_service(cs_uuid):
    ...
    for cs_item in e2e_cs_json["domain-cs"]:
        ...
        if cs_item["address-owner"] == str(settings.web3.eth.defaultAccount):
            settings.logger.debug("ORCH: Sending domain CS request to the local SDN controller.")
            settings.logger.info("TIME TERMINATE LOCAL - " + str(cs_item["uuid"]) + " - " + str(datetime.now()))
            response = sdn_mapper.terminate_connectivity_service(cs_item["uuid"])
            settings.logger.info("TIME TERMINATE LOCAL - " + str(cs_item["uuid"]) + " - " + str(datetime.now()))
            if response[1] == 200 and response[0]["status"] == "TERMINATED":
                #update local domain CS information
            ...
            settings.logger.debug("ORCH: Distribute domain CS request to terminate in the Blockchain.")
            settings.logger.info("TIME TERMINATE BLOCKCHAIN - " + str(cs_item["uuid"]) + " - " + str(datetime.now()))
            response = bl_mapper.terminate_blockchain_cs(cs_item["address-owner"], cs_item["uuid"])
    ...
    for idx, route_item in enumerate(route_nodes_list):
        if route_item["sip_uuid"] == "" and route_item["nep_direction"] == "OUTPUT":
            settings.logger.info("ORCH: Updating Internal OUTPUT (Transmitter) NEP info.")
            # updates the internal NEPsinformation
            # gets the nep info
            ...
            settings.logger.info("ORCH: Updating Internal INPUT (Receiver) NEP info.")
            continue
        else:
...
def instantiate_e2e_slice(incoming_data):
    ...
    for connection_item in incoming_data['instantiation_params']['slice_vl']:
        ...
        for index_item, node_item in enumerate(path_nodes_list):
            settings.logger.debug("ORCH: ******** node_item --> " +str(node_item))
            ...
            if node_item != path_nodes_list[-1]:
                for context_item in contexts_list:
                    if node_item == context_item["topology"]["node_name"]:
                        if index_item == 0:   # the first cs depends on the source sip
                            ...
                            settings.logger.debug("ORCH: ******** A MIDDLE NODE IN THE PATH ********")
                            for port_item in context_item["topology"]["ports"]:
                                settings.logger.info("ORCH: cs_list: " +str(cs_list))
                ...
                settings.logger.debug("ORCH: ******** LAST node_item ********")
